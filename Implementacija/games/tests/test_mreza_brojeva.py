...
async def _get_users():
    blue_user = await User.objects.aget(username='sanja')
    orange_user = await User.objects.aget(username='tanja')
...
async def _play_mreza_brojeva_round1(blue, orange):
...
async def _play_mreza_brojeva_round2(blue, orange):
    await _both_await_msg(blue, orange, 'update_ui')
    await _both_await_msg(blue, orange, 'update_timer')
...
async def test_mreza_brojeva():
    ...
    await _play_mreza_brojeva_round1(blue, orange)
    await _play_mreza_brojeva_round2(blue, orange)
